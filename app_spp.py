...
with st.form("form_input"):
    teks_input = st.text_area("Masukkan teks pesan dari orang tua:", height=120)
    keterangan_bulan = st.text_input("Keterangan bulan (contoh: Januari):")
    submit = st.form_submit_button("Analisis & Simpan ke Riwayat")

if submit:
    if teks_input.strip() and keterangan_bulan.strip():
        hasil = analisis_pesan(teks_input, keterangan_bulan)
        if hasil:
